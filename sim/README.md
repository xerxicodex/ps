...
[![npm version](https://img.shields.io/npm/v/@xerxicodex/sim.svg)](https://www.npmjs.com/package/@xerxicodex/sim)
...
- be usable as a typed and verisoned module that exports a
  [`@xerxicodex/dex-types`](../dex/types)-compatible data layer
...
[`@xerxicodex/data`](../data) (or [`@xerxicodex/dex`](../dex)).**

## Installation

```sh
$ npm install @xerxicodex/sim
```

## Usage

```ts
import {Dex, BattleStreams, RandomPlayerAI, Teams} from '@xerxicodex/sim';
...
- **only Gens 1 - 8 are supported, no other mods**. Generation 8 as the `base` mod is supported as
  ...
  Let's Go, or pet mods). However, the [`@xerxicodex/mods`](../mods) package provides the information
  ...
  `Dex`.
- **random battles are not supported by the `@xerxicodex/sim` package**. All team generation logic and
  data has been removed from the package and are instead to be provided by the
  [`@xerxicodex/randoms`](../randoms) package which exports a generator factory that can be configured
...
- **`Teams` methods delegate to [`@xerxicodex/sets`](../sets)**. In Pokémon Showdown this logic is copied
  between the client and server, but in `@pkmn` has been extracted into a module.
- **`dataDir` and `dataSearch`  have been removed**.
- in order to be [`@xerxicodex/dex-types`](../dex/types) compatible, the **asynchronous `learnsets` API
  has been added** (though unlike like with [`@xerxicodex/dex`](../dex), the learnsets data is loaded at
  ...
  important to **note that the types in `@xerxicodex/sim` do not match the stricter `@xerxicodex/dex-types`**
  directly, you must simply cast the `@xerxicodex/sim` `Dex` and trust that it will work.
- **`DexConditions` has been modified to behave like the client (and `@xerxicodex/dex`)**. If an effect
