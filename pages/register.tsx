...
export default function RegisterPage() {
    ...
    return (
        <div id="register-page" className="w-screen h-screen flex justify-center items-center">
            <div className="bg-white border-b-4 border-blue-500 w-3/4 lg:w-1/3 rounded-md">
                ...
                <FormProvider {...methods}>
                    <form onSubmit={handleSubmit(onSubmitHandler)} className="p-8 pb-4">
                        ...
                        <div className="mb-6">
                            <FormInput label="Confirm Password" name="passwordConfirm" type="password"  placeholder="******************" />
                        </div>

                        <div className="mb-6">
                            <div className="block mb-4">
                                <label className="text-gray-700 text-sm font-bold" htmlFor="starter">
                                    Select Starter
                                </label>
                                <FormInput hidden={true} name="starter" type="number" value={starter?.id ?? 0} />
                            </div>
                            <div className="grid grid-flow-col grid-cols-3 gap-x-[4px]">
                                {starters.map(_starter => {
                                    return (
                                        <div key={_starter.id} className={classnames(["p-4 border flex justify-center items-center font-lighter", starter.id == _starter.id ? "bg-green-200 border-green-400" : "cursor-pointer bg-white hover:bg-white/75 active:bg-white/50 text-gray-500"])} onClick={() => setStarter(_starter)}>
                                            {_starter.name}
                                        </div>
                                    )
                                })}
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <button disabled={isSubmitting} className="bg-indigo-500 hover:bg-indigo-700 disabled:bg-opacity-25 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" type="submit">
                                Register
                            </button>
                        </div>
                    </form>
